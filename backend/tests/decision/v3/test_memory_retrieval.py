...
def test_retrieve_memory_does_not_write_metrics_by_default() -> None:
    agent: dict = {}
    _add(agent, "m1", tags=("trader",), last_accessed_turn=None)
    _ = retrieve_memory(agent, MemoryQuery(purpose="p", tags=("trader",)), world_turn=123)
    assert "retrieval_metrics" not in agent["memory_v3"]["stats"]


def test_retrieve_memory_can_write_metrics_when_enabled() -> None:
    agent: dict = {}
    _add(agent, "m1", tags=("trader",), last_accessed_turn=None)
    _ = retrieve_memory(
        agent,
    ...
    assert metrics["memory_retrieval_calls"] == 1
