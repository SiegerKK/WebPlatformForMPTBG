...
## 4b. Цепочка «get_rich» — логика поиска артефактов
...
### Фаза поиска и сбора артефактов

Сталкер **не знает** заранее, где находятся артефакты — он обследует аномальные зоны
и ориентируется только на память (`explore_confirmed_empty`).  
Радиус поиска ограничен навыком: **1 + `skill_survival`** переходов (например, 2 для 1-го уровня).

| Шаг | Триггер | Действие | Запись в память |
|-----|---------|----------|-----------------|
| 1 | Текущая локация с аномальной активностью, ещё не исследована | Провести разведку (30 тиков) | `"На локации X есть аномальная активность — нужно провести разведку чтобы найти артефакты."` |
| 2 | Текущая локация пуста или без аномалий | BFS в радиусе `1+skill_survival` хопов → выбрать лучшую неисследованную зону | `"Иду в непроверенную аномальную зону «X»."` |
| 3 | Все зоны в радиусе подтверждены пустыми | Идти в ближайшую аномальную зону ждать спауна | `"Все аномальные зоны изучены — иду в «X» ждать."` |
| 4 | Нет аномалий в радиусе | Перейти в соседнюю локацию с наибольшей аномальной активностью | `"Иду в зону с высокой аномальностью."` |

### Фаза продажи артефактов

| Шаг | Триггер | Действие | Запись в память |
|-----|---------|----------|-----------------|
| 5 | Артефакт найден (в инвентаре) и есть торговец в локации | Продать все артефакты торговцу | `"Продал N артефактов на M денег."` |
| 6 | Артефакт найден, торговца нет рядом | BFS → ближайший торговец | `"Ближайший торговец: <name> в <location>."` |
| 7 | Торговец найден | travel к торговцу (хоп за хопом) | `"Иду к торговцу <name>."` |

> **Примечание**: каждый travel реализован как последовательность одиночных хопов.
> Агент реально оказывается в каждой промежуточной локации.
