...
def evaluate_item_needs(ctx: AgentContext, state: dict[str, Any]) -> list[ItemNeed]:
    ...
    needs: list[ItemNeed] = [
        _build_stock_need("food", desired_food, food_count, 0.55, FOOD_ITEM_TYPES, priority=30, reason="Недостаточный запас еды"),
        _build_stock_need("drink", desired_drink, drink_count, 0.55, DRINK_ITEM_TYPES, priority=20, reason="Недостаточный запас воды"),
        _build_stock_need("medicine", desired_medicine, medicine_count, 0.45, HEAL_ITEM_TYPES, priority=60, reason="Недостаточный запас медикаментов"),
    ]
    ...
    needs.append(
        ItemNeed(
            ...
            expected_min_price=_min_buy_price(WEAPON_ITEM_TYPES),
        )
    )
    ...
    needs.append(
        ItemNeed(
            ...
            expected_min_price=_min_buy_price(ARMOR_ITEM_TYPES),
        )
    )
    ...
    needs.append(
        ItemNeed(
            ...
            expected_min_price=_min_buy_price(ammo_types) if ammo_types else None,
        )
    )
...
def _build_stock_need(
    ...
    *,
    ...
    return ItemNeed(
        ...
        expected_min_price=_min_buy_price(item_types),
    )
