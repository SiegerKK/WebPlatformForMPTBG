...
def emit_event(match_id: uuid.UUID, context_id: uuid.UUID, event_type: str, payload: dict, caused_by: uuid.UUID, db: Session) -> GameEvent:
    ...
    event = GameEvent(
        ...
        payload=payload,
        caused_by_command_id=caused_by,
        sequence_number=seq,
    )
    db.add(event)
    db.commit()
...
def get_next_sequence_number(context_id: uuid.UUID, db: Session) -> int:
    result = db.query(func.max(GameEvent.sequence_number)).filter(GameEvent.context_id == context_id).scalar()
    return (result or 0) + 1

def get_match_events(match_id: uuid.UUID, db: Session) -> List[GameEvent]:
    return db.query(GameEvent).filter(GameEvent.match_id == match_id).order_by(GameEvent.sequence_number).all()

def get_context_events(context_id: uuid.UUID, db: Session) -> List[GameEvent]:
    return db.query(GameEvent).filter(GameEvent.context_id == context_id).order_by(GameEvent.sequence_number).all()
